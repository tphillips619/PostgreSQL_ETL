...
def process_song_file(cur, filepath):
    """Process a single song_file and insert it's into a PostgreSQL table"""
    # open song file
...
def process_log_file(cur, filepath):
    """Process a single log_file and insert it's into a PostgreSQL table"""
    # open log file
    df =  pd.read_json(filepath, lines=True)
    ...
    time_data = (df['ts'], df['ts'].dt.hour, df['ts'].dt.day, 
                 df['ts'].dt.week, df['ts'].dt.month, df['ts'].dt.year,
                 df['ts'].dt.weekday)
...
def process_data(cur, conn, filepath, func):
    """Open filepath and walk recursively through all files"""
